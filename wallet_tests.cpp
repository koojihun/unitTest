...
BOOST_AUTO_TEST_CASE(coin_selection_tests)
{
	...
	for (int i = 0; i < RUN_TESTS; i++)
	{
		...
		// 아직 add_prod를 하지 않았기 때문에 tmp_1은 찾을 수 없어야 한다.
		CTxInfo tmp_1(1, 1, 1);
		// SelectCoinsMinConf() -> transaction을 위해 내가 가진 coin들 중 사용할 coin을 결정하는 함수.
		BOOST_CHECK(!wallet.SelectCoinsMinConf(tmp_1, vCoins, setCoinsRet));


		add_prod(1, 1, 1, 4);        // add a new 1 cent coin

		// with a new 1 cent coin, we still can't find a mature 1 cent --> 이게 아니고
		// {ID = 1, CountryCode = 1, ZipCode = 1} 인 상품을 추가했으니 true가 나와야 한다.
		BOOST_CHECK(wallet.SelectCoinsMinConf(tmp_1, vCoins, setCoinsRet));
		BOOST_CHECK_EQUAL(nValueRet, 1 * CENT);

		add_prod(2, 2, 2, 4);           // {id = 2, countryCode = 2, zipCode = 2}인 상품을 추가!!!!

		// we can't make 3 cents of mature coins
		// {id = 3, countryCode = 3, zipCode = 3}인 상품은 없어야한다!!!!
		CTxInfo tmp_3(3, 3, 3);
		BOOST_CHECK(!wallet.SelectCoinsMinConf(tmp_3, vCoins, setCoinsRet));

		// we can make 3 cents of new  coins
		//		//BOOST_CHECK( wallet.SelectCoinsMinConf( 3 * CENT, 1, 1, vCoins, setCoinsRet, nValueRet));
		//		//BOOST_CHECK_EQUAL(nValueRet, 3 * CENT);
		//		BOOST_CHECK(wallet.SelectCoinsMinConf(2 * CENT, 1, 6, vCoins, setCoinsRet, nValueRet));
		//		BOOST_CHECK_EQUAL(nValueRet, 2 * CENT);
		//
		...
		//		BOOST_CHECK(wallet.SelectCoinsMinConf(10 * CENT, 1, 6, vCoins, setCoinsRet, nValueRet));
		//		BOOST_CHECK_EQUAL(nValueRet, 10 * CENT);
		//		
		//		BOOST_CHECK(wallet.SelectCoinsMinConf(5 * CENT, 1, 6, vCoins, setCoinsRet, nValueRet));
		//		BOOST_CHECK_EQUAL(nValueRet, 5 * CENT);
		//
		//		BOOST_CHECK(wallet.SelectCoinsMinConf(20 * CENT, 1, 6, vCoins, setCoinsRet, nValueRet));
		...
		//
		//		// Ravic: Let's start from here again
